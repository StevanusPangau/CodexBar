...
extension CostUsageScanner {
    ...
    static func loadCodexPriorityTurnsMemoFromDiskIfNeeded(cacheRoot: URL? = nil) {
        ...
        for (path, persistedState) in persisted {
            var state = persistedState
            // Observation IDs only order overlapping scans within one process.
    ...
    static func persistCodexPriorityTurnsMemoIfDirty(cacheRoot: URL? = nil) {
        ...
        let snapshot = self.codexPriorityTurnsMemoSnapshot()
...
extension CostUsageScanner {
    static func loadCodexPriorityTurnsMemoFromDiskIfNeeded(cacheRoot: URL? = nil) {}
    static func persistCodexPriorityTurnsMemoIfDirty(cacheRoot: URL? = nil) {}
    static func invalidateCodexPriorityTurnsMemo(forPath _: String) {}
}
#endif
